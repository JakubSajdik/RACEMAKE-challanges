...
function analyzeLap(
  ...
  // Sort by delta — worst sector first (largest time loss)
...
/**
 * Compare two lap analyses and describe how issues evolve over a stint.
 */
function generateStintSummary(
  first: LapAnalysis,
  second: LapAnalysis,
  config: Config
): string {
  const deltaTrend = second.totalDelta - first.totalDelta;
  const lines: string[] = [];

  if (deltaTrend > 0.3) {
    lines.push(
      `Total time lost vs reference grew by about ${deltaTrend.toFixed(2)}s from lap 1 to lap 2 — pace is sliding as the stint goes on.`
    );
  } else if (deltaTrend < -0.05) {
    lines.push(
      "Total delta improved on the second lap — you're finding rhythm or conditions helped."
    );
  }

  const bySector = (a: LapAnalysis) =>
    new Map(a.findings.map((f) => [f.sectorKey, f]));

  const m1 = bySector(first);
  const m2 = bySector(second);

  const tractionWorse: number[] = [];
  const earlyLiftEmerging: number[] = [];
  ...
  for (const key of m1.keys()) {
    ...
    if (
      f2.issue === "traction_loss" &&
      (f1.issue !== "traction_loss" || f2.delta > f1.delta + 0.2)
    ) {
      tractionWorse.push(f1.sector);
    }
    if (f2.issue === "early_lift" && f1.issue !== "early_lift") {
      earlyLiftEmerging.push(f2.sector);
    }
  }

  if (tractionWorse.length) {
    lines.push(
      `Traction loss is worsening in sector(s) ${tractionWorse.join(", ")} — more slip and TC intervention on lap 2. Ease onto the throttle and use the grip you have.`
    );
  }

  if (earlyLiftEmerging.length) {
    lines.push(
      `Early lift shows up on lap 2 in sector(s) ${earlyLiftEmerging.join(", ")} — classic tyre-management tell; you're protecting entry or saving rears.`
    );
  }

  if (!lines.length) {
    lines.push(
      "Pattern across laps: issue mix is similar; focus stays on the biggest per-lap deltas below."
    );
  }
...
function analyzeStint(
  reference: ReferenceLap,
  driverLaps: { label: string; lap: DriverLap }[],
  config: Config
): StintReport {
  const laps: PerLapCoaching[] = driverLaps.map(({ label, lap }) => {
    const analysis = analyzeLap(reference, lap);
    return {
      lapLabel: label,
      analysis,
      coaching: generateCoaching(analysis, config),
    };
  });

  let stintSummary = "";
  if (laps.length >= 2) {
    stintSummary = generateStintSummary(
      laps[0]!.analysis,
      laps[laps.length - 1]!.analysis,
      config
    );
  } else if (laps.length === 1) {
    stintSummary =
      config.coachVoice === "pitgpt"
        ? "One lap in this stint — add another lap to see how issues evolve."
        : "Single lap provided; stint trends require at least two laps.";
  }
...
const stint = analyzeStint(
  referenceLap,
  [
    { label: "Stint lap 1", lap: driverLap },
    { label: "Stint lap 14", lap: driverLap2 },
  ],
  config
);

const result = stint.laps[0]?.coaching;

console.log("--- PitGPT Lap Analysis (per lap) ---");
if (stint.laps.length === 0) {
  console.log("(no laps provided)");
} else {
  for (const { lapLabel, coaching } of stint.laps) {
    console.log(`\n## ${lapLabel}`);
    console.log(JSON.stringify(coaching, null, 2));
  }
}
console.log("\n--- Stint summary ---");
console.log(stint.stintSummary || "(no summary)");
console.log("---------------------------");

// --- Validation (lap 1 / original pipeline) ---
if (!result) {
  console.log("\n❌ No lap data — skipping validation.");
} else {
  const checks = [
    { name: "problemSector", pass: result.problemSector === 2 },
    {
      name: "issue",
      pass: (["late_braking", "traction_loss"] as string[]).includes(
        result.issue
      ),
    },
...
/*
 * LEVEL 3 — Production scale (brief)
 * ------------------------------------
 * One car → full session: I'd move analysis off the hot path — ingest 120 Hz
 * streams into time-series storage, downsample or window for sector aggregates,
 * and process laps asynchronously (queue/worker per session). The first things
 * that break are: single-threaded in-memory processing (CPU + latency), lack of
 * backpressure (memory if analysis can't keep up with ingest), and conflating
 * all cars in one pipeline (no isolation — one slow car blocks others). I'd add
 * per-car pipelines, bounded buffers, and precomputed reference baselines so
 * coaching reads cheap snapshots instead of replaying full telemetry each time.
 */