 ...
 * HOW TO RUN
 * ----------
 *   From this folder (Task_basic):
 *   bun run challenge.ts       (preferred — Racemake uses Bun)
...
/**
 * Compare first vs last lap in a stint and describe how issues evolve.
 */
function generateStintSummary(
  first: PerLapCoaching,
  last: PerLapCoaching,
  config: Config
): string {
  const firstA = first.analysis;
  const secondA = last.analysis;
  const firstLabel = first.lapLabel;
  const lastLabel = last.lapLabel;
  const deltaTrend = secondA.totalDelta - firstA.totalDelta;
  const lines: string[] = [];

  if (deltaTrend > 0.3) {
    lines.push(
      `Total time lost vs reference grew by about ${deltaTrend.toFixed(2)}s from ${firstLabel} to ${lastLabel} — pace is sliding as the stint goes on.`
    );
  } else if (deltaTrend < -0.05) {
    lines.push(
      `Total delta improved from ${firstLabel} to ${lastLabel} — you're finding rhythm or conditions helped.`
    );
  }
  ...
  const m1 = bySector(firstA);
  const m2 = bySector(secondA);
  ...
  if (tractionWorse.length) {
    lines.push(
      `Traction loss is worsening in sector(s) ${tractionWorse.join(", ")} — more slip and TC intervention on ${lastLabel}. Ease onto the throttle and use the grip you have.`
    );
  }

  if (earlyLiftEmerging.length) {
    lines.push(
      `Early lift shows up on ${lastLabel} in sector(s) ${earlyLiftEmerging.join(", ")} — classic tyre-management tell; you're protecting entry or saving rears.`
    );
  }
